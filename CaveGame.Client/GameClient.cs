...
public class GameClient : IGameContext, IGameClient
{
	...
	public GameClient(CaveGameDesktopClient _game)
	{
		...
		ClientTasks = new List<RepeatingIntervalTask>
		{
			...
			new RepeatingIntervalTask(World.UnloadOutOfRangeChunks, 1 / 4.0f),
	...
	{
		Logger.LogClient("Ping from server");
	}
	...
	private void SpawnBomb(int ID) => World.Entities.Add(new BombEntity
	{
		EntityNetworkID = ID
	});
	private void SpawnWurmhole(int ID) => World.Entities.Add(new WurmholeEntity
	{
		EntityNetworkID = ID
	});
	private void SpawnArrow(int ID) => World.Entities.Add(new ArrowEntity
	{
		EntityNetworkID = ID
	});
	private void SpawnDynamite(int ID) => World.Entities.Add(new DynamiteEntity
	{
		EntityNetworkID = ID
	});
	...
	private void OnServerHandshakeReply(NetworkMessage msg)
	{
		...
		if (!protocolMatches)
		{
			Logger.LogClient($"Protocol Mismatch: server {packet.ServerProtocolVersion} vs client {Globals.ProtocolVersion} ");
			Game.CurrentGameContext = Game.MenuContext;
		...
		Logger.LogClient($"Server handshake! sprotocol: {packet.ServerProtocolVersion} (matches)");
		Logger.LogClient($"Connecting to {packet.ServerName}");
		Logger.LogClient($"{packet.ServerName}: {packet.ServerMOTD}");
		int currentPlrCount = packet.PlayerList.Where(str => str.Length > 0).Count();
		Logger.LogClient($"Connected players ({currentPlrCount}):");
		foreach (var name in packet.PlayerList.Where(str => str.Length > 0))
			Logger.LogClient($"    {name}");
		Logger.LogClient($"Requesting game session...");
