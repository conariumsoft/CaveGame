...
// FIXME: Should be in CaveGame.Client
// But there seems to be a reference to this somewhere else.
// Which should be fixed aswell.
...
public class CommandBar : GameComponent, IMessageOutlet
{
    ...
    private void CommandList(params string[] args)
    {
        ...
        {
            MessageHistory.Add(new Message(cmd.Keyword + " " + GetArgsInfo(cmd) + " (" + cmd.Description + ")",
                Color.GreenYellow));
        }
    }
    ...
    // TODO: Refactor how these are implemented
    ...
    public void Draw(GraphicsEngine GFX)
    {
        ...
        GFX.OutlineRect(new Color(0.5f, 0.5f, 0.5f), consolePosition - new Vector2(2, 2),
            consoleSize + new Vector2(4, 4));
        ...
        lock (MessageHistory)
        {
            ...
            for (int i = MessageHistory.Count - 1; i >= 0; i--)
            {
                ...
                GFX.Text(font, text, consolePosition + new Vector2(0, (consoleSize.Y - 20) - (visIter * line_height)),
                    message.TextColor);
            }
        }
        ...
        if (inputBox.SpecialSelection == false && (inputBox.InputBuffer != "" || autocompletemenuOpen == true))
        {
            ...
            foreach (Command cmd in Commands)
            {
                ...
                {
                    GFX.Text(font, InputBufferProcess(cmd.Keyword) + " " + GetArgsInfo(cmd), inputBoxPosition,
            ...
            if (autocompleteOptionIndex > 0)
                GFX.Rect(new Color(0.1f, 0.1f, 0.1f),
                    inputBoxPosition - new Vector2(0, (autocompleteOptionIndex + 1) * line_height),
                    new Vector2(400, autocompleteOptionIndex * line_height));
            ...
            foreach (Command cmd in Commands)
            {
                if (cmd.Keyword.StartsWith(cleaned))
                {
                    GFX.Text(font, cmd.Keyword + " " + GetArgsInfo(cmd),
                        inputBoxPosition - new Vector2(-5, (iterator2 + 2) * line_height), new Color(1.0f, 1.0f, 0));
                    iterator2++;
                }
            }
        }
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        if (Open)
        {
            ...
            if (keyboard.IsKeyDown(Keys.Down) && !previousKBState.IsKeyDown(Keys.Down))
            {
                ...
                inputBox.InputBuffer = (inputCurrent == CommandHistory.Count) ? "" : CommandHistory[inputCurrent];
            }
        }
